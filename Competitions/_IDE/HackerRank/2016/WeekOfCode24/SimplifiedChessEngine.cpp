...
// MY
int kf_first, kf_second, mv_limit;
vector<vector<char>> board(4, vector<char>());
int kf [2];

typedef pair<short, vector<vector<char>>> tkey;
typedef pair<short, short> tvalue;
map<tkey, tvalue> memo;

vector<vector<vector<pair<int, int>>>> v;

const int WHITE = 0;
const int BLACK = WHITE ^ 1;
const int NONE = -1;

short play(int mv_k, short who, vector<vector<char>> board) {
    // No Q
    bool hasWq = false, hasBq = false;
    _forn(ii, 0, 4)
        _forn(jj, 0, 4) {
            if ('A' == board [ii][jj]) hasWq = true;
            if ('E' == board [ii][jj]) hasBq = true;
        }
    ...
    // Memo
    tkey memoKey = mp(who, board);
    if (memo.count (memoKey)) {
        tvalue memoVal = memo [memoKey];
        if (mv_k >= memoVal.first) {
            return memoVal.second;
        }
    }

    // Movements
    bool willWWin = BLACK == who;

    bool enough = false;
    _forn(i, 0, 4) {
        _forn(j, 0, 4) {
            if (!(board[i][j] >= 'A' + who * 4
                  && board[i][j] <= 'D' + who * 4))
                continue;

            int baseWho = board[i][j] - ('A' + who * 4);
            _forn(iv, 0, csize (v[baseWho])) {
                _forn(jv, 0, csize (v[baseWho][iv])) {
                    pair< int, int > d = v[baseWho][iv][jv];
                    int ni = i + d.first, nj = j + d.second;
                    if (ni < 0 || ni >= 4 || nj < 0 || nj >= 4) continue;
                    bool ok = ('.' == board[ni][nj]
                               || (board[ni][nj] >= 'A' + (who ^ 1) * 4
                                   && board[ni][nj] <= 'D' + (who ^ 1) * 4));

                    if (ok) {
                        vector< vector< char>> nboard (board);
                        nboard[i][j] = '.', nboard[ni][nj] = board[i][j];
                        short whoWin = play (mv_k + 1, (short) (who ^ 1), nboard);
                        if (BLACK == who)
                            willWWin &= (BLACK != whoWin); else
                            willWWin |= (WHITE == whoWin);
                        if (BLACK == who && !willWWin) {
                            enough = true;
                            break;
                        }
                        if (WHITE == who && willWWin) {
                            enough = true;
                            break;
                        }
                    }
    ...
    // Store & return
    short rtvWin = (short) (willWWin ? WHITE : BLACK);
    memo [memoKey] = mp(mv_k, rtvWin);
    return rtvWin;
}
...
int main() {
    ...
    // Q -> A (E)
    v [0].resize(8);
    v [0][0].pb(mp(-1, -1)); v [0][0].pb(mp(-2, -2)); v [0][0].pb(mp(-3, -3));
    v [0][1].pb(mp(+1, +1)); v [0][1].pb(mp(+2, +2)); v [0][1].pb(mp(+3, +3));
    v [0][2].pb(mp(-1, +1)); v [0][2].pb(mp(-2, +2)); v [0][2].pb(mp(-3, +3));
    v [0][3].pb(mp(+1, -1)); v [0][3].pb(mp(+2, -2)); v [0][3].pb(mp(+3, -3));
    v [0][4].pb(mp(+1, +0)); v [0][4].pb(mp(+2, +0)); v [0][4].pb(mp(+3, +0));
    v [0][5].pb(mp(-1, +0)); v [0][5].pb(mp(-2, +0)); v [0][5].pb(mp(-3, +0));
    v [0][6].pb(mp(+0, +1)); v [0][6].pb(mp(+0, +2)); v [0][6].pb(mp(+0, +3));
    v [0][7].pb(mp(+0, -1)); v [0][7].pb(mp(+0, -2)); v [0][7].pb(mp(+0, -3));
    // N -> B (F)
    v [1].resize(8);
    v [1][0].pb(mp(+1, +2));
    v [1][1].pb(mp(+1, -2));
    ...
    // B -> C (G)
    v [2].resize(4);
    v [2][0].pb(mp(-1, -1)); v [2][0].pb(mp(-2, -2)); v [2][0].pb(mp(-3, -3));
    v [2][1].pb(mp(+1, +1)); v [2][1].pb(mp(+2, +2)); v [2][1].pb(mp(+3, +3));
    v [2][2].pb(mp(-1, +1)); v [2][2].pb(mp(-2, +2)); v [2][2].pb(mp(-3, +3));
    v [2][3].pb(mp(+1, -1)); v [2][3].pb(mp(+2, -2)); v [2][3].pb(mp(+3, -3));
    // R -> D (I)
    v [3].resize(4);
    v [3][0].pb(mp(+1, +0)); v [3][0].pb(mp(+2, +0)); v [3][0].pb(mp(+3, +0));
    v [3][1].pb(mp(-1, +0)); v [3][1].pb(mp(-2, +0)); v [3][1].pb(mp(-3, +0));
    v [3][2].pb(mp(+0, +1)); v [3][2].pb(mp(+0, +2)); v [3][2].pb(mp(+0, +3));
    v [3][3].pb(mp(+0, -1)); v [3][3].pb(mp(+0, -2)); v [3][3].pb(mp(+0, -3));

    // CODE AREA =>
    forT(testId) {
        scanf("%d %d %d\n", &kf_first, &kf_second, &mv_limit);
        kf [0] = kf_first, kf [1] = kf_second;
        _forn(i, 0, 4) board [i].assign(4, '.');
        mv_limit -= (!(mv_limit % 2));

        _forn(i, 0, kf_first + kf_second) {
            char fc, rowc;
            int row, column;
            scanf("%c %c %d\n", &fc, &rowc, &column);

            if (fc == 'Q') fc = (char) ('A' + (i >= kf_first) * 4); else
            if (fc == 'N') fc = (char) ('B' + (i >= kf_first) * 4); else
            if (fc == 'B') fc = (char) ('C' + (i >= kf_first) * 4); else
            if (fc == 'R') fc = (char) ('D' + (i >= kf_first) * 4);
