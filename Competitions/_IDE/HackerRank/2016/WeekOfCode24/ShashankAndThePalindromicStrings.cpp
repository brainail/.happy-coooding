...
int main() {
    ...
    forT(testId) {
        ...
        _forn(i, 0, ns) {
            ...
            s.append (ws), n += lws;
            s.append ("#"); ++ n;
            if (1 == lws) skippable [n - 2] = false;

            fordn (j, n - 1, 0) {
                if (s [j] == '#' && j != n - 1) { break; }
                if (s [j] == '#') { section [j] = -1; continue; }
                section [j] = i + 1;
            }
        }
        s.append("-"), ++ n;
        ...
        _forn(i, 0, n)
            _forn(j, i, n)
                if (s[i] == s[j] && s[i] != '#') {
                    if (section[j] - section[i] > 1) continue;
                    dp[i][1][j][1] = 1;
                }
        ...
        _forn(l, 1, n)
            _forn(i, 0, n) {
                ...
                _forn (ti, 0, 2)
                    _forn (tj, 0, 2) {
                        if (! dp[i][ti][j][tj]) continue;

                        bool canSkipLeft = true;
                        if (!i) canSkipLeft = false;
                        if (i && s [i - 1] == '#' && !ti) canSkipLeft = false;
                        if (canSkipLeft) {
                            int nti = s [i - 1] != '#' ? ti : 0;
                            int ni = s [i - 1] != '#' ? i - 1 : i - 2;
                            bool canGoLeft = true;
                            if (! skippable [ni] && ! nti) canGoLeft = false;
                            if (canGoLeft) {
                                mod_plus (dp[ni][nti][j][tj], dp[i][ti][j][tj]);
                            }
                        }

                        bool canSkipRight = true;
                        if (j + 1 >= n) canSkipRight = false;
                        if (j + 1 < n && s [j + 1] == '#' && !tj) canSkipRight = false;
                        if (canSkipRight) {
                            int ntj = s [j + 1] != '#' ? tj : 0;
                            int nj = s [j + 1] != '#' ? j + 1 : j + 2;
                            bool canGoRight = true;
                            if (! skippable [nj] && ! ntj) canGoRight = false;
                            if (canGoRight) {
                                mod_plus (dp[i][ti][nj][ntj], dp[i][ti][j][tj]);
                            }
                        }

                        if (i && j + 1 < n) {
                            bool canGoLeft = true;
                            if (s [i - 1] == '#' && !ti) canGoLeft = false;
                            int ni = s [i - 1] != '#' ? i - 1 : i - 2;
                            bool canGoRight = true;
                            if (s [j + 1] == '#' && !tj) canGoRight = false;
                            int nj = s [j + 1] != '#' ? j + 1 : j + 2;
                            if (s [ni] == s [nj] && canGoLeft && canGoRight) {
                                int nti = 1, ntj = 1;
                                mod_plus (dp[ni][nti][nj][ntj], dp[i][ti][j][tj]);
                            }
                        }
                    }
            }

        int rtv = dp [0][1][n - 1][1];
        rtv = ((rtv - 1) + MODULO) % MODULO;
        printf("%d\n", rtv);
    }
    // CODE AREA <=
