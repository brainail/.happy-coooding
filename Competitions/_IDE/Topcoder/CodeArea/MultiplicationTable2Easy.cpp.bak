...
class MultiplicationTable2Easy {
    ...
    string isGoodSet (vector <int> table, vector <int> t) {
        int n = (int) sqrt(csize(table));
        int nt = csize(t);
        _forn(i, 0, nt)
            _forn(j, 0, nt) {
                bool nice = false;
                _forn(q, 0, nt)
                    if (table [t [i] * n + t [j]] == t [q]) {
                        nice = true;
                    }
                if (! nice) {
                    return "Not Good";
...
namespace moj_harness {
	...
	int run_test_case(int casenum__) {
		...
		case 0: {
			...
			string received__         = MultiplicationTable2Easy().isGoodSet(vector <int>(table, table + (sizeof table / sizeof table[0])), vector <int>(t, t + (sizeof t / sizeof t[0])));
		...
		case 4: {
			int table[]               = {2,2,2,2,2,2, 2,2,2,2,2,2, 2,2,2,2,2,2, 2,2,2,2,2,2, 2,2,2,2,2,2, 2,2,2,2,2,2};
			int t[]                   = {2,4,5};
			string expected__         = "Good";
