...
map<string, int> chatsTime;
// vector< pair< int, pair<int, string> > > chatsOrder;
vector< pair< int, string> > chatsOrder;
...
//        ++ chats [name];
    ...
    for (it = chatsTime.begin(); it != chatsTime.end(); ++ it) {
        // chatsOrder.pb(mp(it->second, mp(chatsTime [it->first], it->first)));
        chatsOrder.pb(mp(it->second, it->first));
    }

    sort(rall(chatsOrder));

    for (int i = 0, nChats = csize(chatsOrder); i < nChats; ++ i) {
        printf("%s\n", chatsOrder [i].second.c_str());
    }
    // CODE AREA <=
