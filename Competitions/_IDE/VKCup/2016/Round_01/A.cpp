...
const string INPUT_FILE
        = IDE_PATH + "VKCup/2016/Round_01/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "VKCup/2016/Round_01/output.txt";
...
int main() {
    ...
    _forn(_q, 0, q) {
        ...
        if (1 == op) {
            friends.insert(mp(t [fid - 1], fid));
        } else {
            bool ok = false;
            int fk = 0;
            for (set< pair<int, int> >::iterator it = friends.begin(); it != friends.end(); ++ it) {
                if (it->second == fid) ok = true;
                ++ fk;
                if (fk >= k) break;
            }
            printf("%s\n", ok ? "YES" : "NO");
