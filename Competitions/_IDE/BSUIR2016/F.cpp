...
inline void clearq(queue< pair<int, pair<int, int> > > & q) {
    queue< pair<int, pair<int, int> > > empty;
    swap(q, empty);
}

inline void tryPush(pair<int, pair<int, int> > state) {
    int c = state.first, x = state.second.first, y = state.second.second;
    if (x < 1 || y < 1 || x > n || y > m) return;

    if (board [x][y] == -2) return;
    if (board [x][y] != -1 && c <= board [x][y]) return;
...
int main() {
    ...
    _forn(i, 0, n) {
        _forn(j, 0, m) {
            char ch;
            scanf("%c", &ch);
            board [i + 1][j + 1] = '.' == ch ? -1 : -2;
        }
        scanf("\n");
    }

    p.reserve((unsigned long) k);
    ...
    _forn(i, 0, k) {
        ...
        while (! q.empty ()) {
            ...
            tryPush (mp(state.first - 1, mp(state.second.first, state.second.second + 1)));
            tryPush (mp(state.first - 1, mp(state.second.first, state.second.second - 1)));
            tryPush (mp(state.first - 1, mp(state.second.first + 1, state.second.second)));
            tryPush (mp(state.first - 1, mp(state.second.first - 1, state.second.second)));
        }
    }
