...
const string INPUT_FILE
        = IDE_PATH + "FacebookHackerCup/2016/01_Round/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "FacebookHackerCup/2016/01_Round/output.txt";
...
int main() {
    ...
    forT(testId) {
        ...
        _forn(i, 0, N) scanf("%d", &X [i]);

        _forn(i, 0, N) {
            fordn(j, 4, 1)
                if (dp [i][j] >= 0) {
                    // #1 ? ? X_I NEW=X_I+1 NEW=X_I+2 ... NEW=X_I+Y (Y<4)
                    if (X [i] + (j - 1) <= MAXV) {
                        int val = dp [i][j] + (j - 1);
                        if (dp[i + 1][4] < 0) dp [i + 1][4] = val;
                        dp [i + 1][4] = min (dp [i + 1][4], val);
                    }

                    // #2 ? ? X_I NEW_0=? NEW_1=? ... NEW_K=? X_(I+1) (K<3)
                    if (i + 1 < N) {
                        forn(ja, 0, min(2, j - 2))
                            if (X [i + 1] - X [i] > ja && X [i + 1] - X [i] <= (ja + 1) * 10) {
                                int val = dp [i][j] + ja;
                                if (dp [i + 1][j - 1 - ja] < 0) dp [i + 1][j - 1 - ja] = val;
                                dp [i + 1][j - 1 - ja] = min (dp [i + 1][j - 1 - ja], val);
                            }
                    }
                }
