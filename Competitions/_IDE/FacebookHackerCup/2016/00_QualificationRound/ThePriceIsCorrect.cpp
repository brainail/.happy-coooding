...
const string INPUT_FILE
        = IDE_PATH + "FacebookHackerCup/2016/00_QualificationRound/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "FacebookHackerCup/2016/00_QualificationRound/output.txt";
...
int main() {
    ...
    forT(testId) {
        ...
        _forn(i, 1, N + 1) getline(cin, w [i]);
        sort(w.begin() + 1, w.begin() + N + 1);
        _forn(i, 1, N + 1) wl [i] = clen(w [i]);

        _forn(i, 1, N) {
            int j = 0, l = min (wl [i], wl [i + 1]);
            while (j < l && w [i][j] == w [i + 1][j]) ++ j;
            eqw [i] = j;
        }

        _forn(i, 1, N + 1) {
            int clnote = wl [i], cfoo = wl [i];
            _forn(j, i, N + 1) {
                // print
                ++ cfoo;
                // empty
                foo [i][j] = cfoo + clnote;
                // eq
                cfoo += abs (clnote - eqw [j]);
                clnote = eqw [j];
                // type
                cfoo += abs (wl [j + 1] - clnote);
                clnote = wl [j + 1];
            }
        }
        ...
        _forn(i, 0, N)
            _forn(k, 0, K)
                  if (dp [i][k] >= 0) {
                      if (dp [i + 1][k] < 0)
                          dp [i + 1][k] = dp [i][k]; else
                          dp [i + 1][k] = min (dp [i + 1][k], dp [i][k]);
                      _forn(j, i + 1, N + 1) {
                          if (k + (j - i) > K) break;
                          int dpval = dp[i][k] + foo[i + 1][j];
                          if (dp[j][k + 1] < 0)
                              dp[j][k + 1] = dpval; else
                              dp[j][k + 1] = min(dp[j][k + 1], dpval);
                      }
                  }
