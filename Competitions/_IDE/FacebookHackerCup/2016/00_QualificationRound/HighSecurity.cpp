...
const string INPUT_FILE
        = IDE_PATH + "FacebookHackerCup/2016/QualificationRound/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "FacebookHackerCup/2016/QualificationRound/output.txt";
...
inline void markG(int i, int j) {
    if ('G' != board [i][j]) return;
    int jj = j;
    while (jj > 0) {
        if ('X' == board [i][jj]) break;
        if ('.' != board [i][jj]) { -- jj; continue; }
        board [i][jj] = '#', -- jj;
    }
    jj = j;
    while (jj < N + 1) {
...
int main() {
    ...
    forT(testId) {
        ...
        board [0].assign(N + 2, 'X');
        board [1].assign(N + 2, 'X');
        ...
        _forn(i, 0, 2)
            _forn(j, 1, N + 1)
                if ('.' == board [i][j] && 'X' == board [i][j - 1] && 'X' == board [i][j + 1]) {
                    board [i][j] = 'X' != board [i ^ 1][j] ? '#' : 'G';
                    board [i ^ 1][j] = 'X' != board [i ^ 1][j] ? 'G' : 'X';
                    markG(i, j);
                    markG(i ^ 1, j);
                }
...
//        _forn(i, 0, 2) {
//            _forn(j, 1, N + 1) printf("%c", ('#' != board [i][j] ? board [i][j] : '.'));
//            printf("\n");
//        }
