...
// MY
const static int r = 50;
const static tld reps = 1e-6;
...
string calcTest() {
    ...
    if (_dist (0, 0, x, y) > sqr (r)) {
        return "white"; // early exit, not in the our circle
    }

    tld angle = 0;

    int x0 = 0, y0 = r;
    angle += acos ((_dist (x0, y0, x, y) - sqr (r) - _dist(0, 0, x, y)) / (-2 * r * dist(0, 0, x, y)));
    ...
    tld pangle = 360 * p / 100.0;

    return pangle >= (angle - reps) ? "black" : "white";
}
