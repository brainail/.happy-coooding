...
template<class T> inline T _dist(T X0, T Y0, T X1, T Y1) { return sqr (X0 - X1) + sqr (Y0 - Y1); }
template<class T> inline double dist(T X0, T Y0, T X1, T Y1) { return sqrt (_dist (X0, Y0, X1, Y1)); }
...
const string INPUT_FILE
        = "/Users/brainail/DevProjects/Privacy/Programming/Happy-coding/Competitions/_IDE/FacebookHackerCup/input.txt";
const string OUTPUT_FILE
        = "/Users/brainail/DevProjects/Privacy/Programming/Happy-coding/Competitions/_IDE/FacebookHackerCup/output.txt";

const static int MAXV = 100 + 10;
const static int MAXF = 5000 + 10;

inline int best (int x, int pivot) {
    return (pivot < 0 || x < pivot) ? x : pivot;
}

int v, e, f;
int g [MAXV][MAXV];
int dp [MAXF][2];
int f_s [MAXF], f_d [MAXF];
...
int calcTest() {
    ...
    _forn(t, 0, v)
        _forn(i, 0, v)
            _forn(j, 0, v)
                if (g [i][t] >= 0 && g [t][j] >= 0) {
                    g[i][j] = best (g[i][t] + g[t][j], g[i][j]);
                }

    f_s [0] = f_d [0] = 0;
    forn(i, 1, f) {
        scanf("%d %d", &f_s [i], &f_d [i]);
        -- f_s [i], -- f_d [i];
    }
    ...
    _forn(lf, 0, f)
        _forn(kb, 0, 2) {
            // early skip, isn't possible to reach this state
            if (dp [lf][kb] < 0) {
                continue;
            }
            ...
            // what kind of trip are we able to do?
            // 1) kb = 0 # lv -> f_s [lf + 1] -> f_d [lf + 1] # kb = 0
            bool isPossibleTrip_01 = (0 == kb);
            isPossibleTrip_01 &= (lf + 1 <= f);
            isPossibleTrip_01 &= (g [lv][f_s [lf + 1]] >= 0);
            isPossibleTrip_01 &= (g [f_s [lf + 1]][f_d [lf + 1]] >= 0);
            // 2) kb = 0 # lv -> f_s [lf + 1] -> f_s [lf + 2] -> f_d [lf + 1] # kb = 1
            bool isPossibleTrip_02 = (0 == kb);
            isPossibleTrip_02 &= (lf + 2 <= f);
            isPossibleTrip_02 &= (g [lv][f_s [lf + 1]] >= 0);
            isPossibleTrip_02 &= (g [f_s [lf + 1]][f_s [lf + 2]] >= 0);
            isPossibleTrip_02 &= (g [f_s [lf + 2]][f_d [lf + 1]] >= 0);
            // 3) kb = 1 # lv -> f_d [lf + 1] # kb = 0
            bool isPossibleTrip_03 = (1 == kb);
            isPossibleTrip_03 &= (lf + 1 <= f);
            isPossibleTrip_03 &= (g [lv][f_d [lf + 1]] >= 0);
            // 4) kb = 1 # lv -> f_s [lf + 2] -> f_d [lf + 1] # kb = 1
            bool isPossibleTrip_04 = (1 == kb);
            isPossibleTrip_04 &= (lf + 2 <= f);
            isPossibleTrip_04 &= (g [lv][f_s [lf + 2]] >= 0);
            isPossibleTrip_04 &= (g [f_s [lf + 2]][f_d [lf + 1]] >= 0);

            // ready steady go!
            // 1)
            if (isPossibleTrip_01) {
                int val = dp [lf][kb] + g [lv][f_s [lf + 1]] + g [f_s [lf + 1]][f_d [lf + 1]];
                dp [lf + 1][kb] = best (val, dp [lf + 1][kb]);
            }
            // 2)
            if (isPossibleTrip_02) {
                int val = dp [lf][kb] + g [lv][f_s [lf + 1]] + g [f_s [lf + 1]][f_s [lf + 2]] + g [f_s [lf + 2]][f_d [lf + 1]];
                dp [lf + 1][kb + 1] = best (val, dp [lf + 1][kb + 1]);
            }
            // 3)
            if (isPossibleTrip_03) {
                int val = dp [lf][kb] + g [lv][f_d [lf + 1]];
                dp [lf + 1][kb - 1] = best (val, dp [lf + 1][kb - 1]);
            }
            // 3)
            if (isPossibleTrip_04) {
                int val = dp [lf][kb] + g [lv][f_s [lf + 2]] + g [f_s [lf + 2]][f_d [lf + 1]];
                dp [lf + 1][kb] = best (val, dp [lf + 1][kb]);
            }
        }
