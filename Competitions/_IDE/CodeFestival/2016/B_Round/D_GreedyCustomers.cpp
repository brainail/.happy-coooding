...
const string INPUT_FILE
        = IDE_PATH + "CodeFestival/2016/B_Round/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "CodeFestival/2016/B_Round/output.txt";
...
int main() {
    ...
    _forn(i, 0, n) scanf("%d", &a [i + 1]);

    tll rtv = 0;
    tll minp = 0;

    _forn(i, 0, n) {
        if (a[i] < a[i + 1] - 1 && a[i + 1] - 1 > minp) {
            tll p = max ((tll) a[i] + 1, minp + 1);
            tll o = a[i + 1];
            tll k = o / p;
            if (!(a[i + 1] - p * k)) --k;
            rtv += k;
