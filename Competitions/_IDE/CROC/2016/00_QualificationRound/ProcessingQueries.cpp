...
int Q, B, qi, ti;
vector< tpii > queries;
queue< pair<tpii, int> > qq;
pair< tpii, int > curQ;
vector< int > rtv;

void processQueue() {
    // nothing here
    if (curQ.first.first == NULO) return;
}
...
int main() {
    ...
    while (qi < Q) {
        ...
        // server is busy, queue has no space
        if (curQ.first.first != NULO && qq.size() >= B) {
            rtv [qi] = -1;
            continue;
        }

        // server is busy and we're able to put some stuff into queue
        if (curQ.first.first != NULO) {
            qq.push(mp(queries [qi], qi));
            continue;
        }
    }
