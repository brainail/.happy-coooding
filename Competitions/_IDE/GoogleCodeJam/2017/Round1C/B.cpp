...
// MY
int dp [2][1440 + 10][720 + 10];
int ft [2][1440 + 10];
bool ff [2][1440 + 10];
int n [2];

int solve(int midnight) {
    fill0 (dp);
    dp [midnight][0][0] = 1;
    ...
    _forn(t, 0, 1440) {
        _forn(last, 0, 2) {
            ...
            forn(lt, 0, 720)
                if (dp[last][t][lt] > 0) {
                    ct[last] = lt, ct[next] = (t > 0 ? ft[last][t - 1] : 0) - lt;
                    forn(nt, t + 1, 1440) {
                        int pt = ct[next] + (nt - t);
                        if (pt > 720) break;
                        if (!ff[last][nt]) continue;
                        if (!ff[next][nt - 1]) break;
                        if (!dp[next][nt][pt]) dp[next][nt][pt] = INF;
                        dp[next][nt][pt] = min (dp[next][nt][pt], dp[last][t][lt] + 1);
                    }
                }
        }
    }

    if (!dp [midnight][1440][720]) dp [midnight][1440][720] = INF;
    return dp [midnight][1440][720] - 1;
}
...
int main() {
    ...
    forT(testId) {
        ...
        vector< pair <int, int> > at [2];
        at [0].clear();
        at [0].resize(n [0]);
        at [1].clear();
        at [1].resize(n [1]);

        _forn(i, 0, n [0]) scanf("%d %d", &at [0][i].first, &at [0][i].second);
        _forn(i, 0, n [1]) scanf("%d %d", &at [1][i].first, &at [1][i].second);
        ...
        _forn(who, 0, 2) {
            _forn(i, 0, n [who]) {
                _forn(t, at [who][i].first, at[ who][i].second) {
                    ft [who][t] = -1;
                    ff [who][t] = false;
                }
            }

            forn(i, 0, 1440) ft [who][i] = ft [who][i] < 0 ? 0 : 1;
            forn(i, 1, 1440) ft [who][i] += ft [who][i - 1];
        }


        int rtv = min(solve(0), solve(1));
