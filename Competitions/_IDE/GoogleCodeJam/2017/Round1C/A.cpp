...
int main() {
    ...
    forT(testId) {
        ...
        _forn(i, 0, n) {
            dp [i][1] = (double) (2 * PI * pan[i].first * pan[i].second);
        }

        _forn(last, 0, n)
            _forn(ct, 1, k)
                _forn(next, last + 1, n) {
                    double news = dp [last][ct];
                    news += (2 * PI * pan [next].first * pan [next].second);
                    news += (PI * pan[last].first * pan[last].first) - (PI * pan[next].first * pan[next].first);
                    dp [next][ct + 1] = max(dp [next][ct + 1], news);
                }

        double rtv = 0;
        _forn(i, 0, n) {
            rtv = max(rtv, dp [i][k] + (double) (PI * pan[i].first * pan[i].first));
        }
