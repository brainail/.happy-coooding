...
const string INPUT_FILE
        = IDE_PATH + "GoogleCodeJam/2017/Round1A/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "GoogleCodeJam/2017/Round1A/output.txt";
...
void seek(int p, int k) {
    ...
    int x = children [p].first;
    int y = children [p].second;
    _forn(u,0,n) {
        if (x-u<0) break;
        _forn(d,0,n) {
            if (x+d>=n) break;
...
int main() {
    ...
    forT(testId) {
        ...
        printf("Case #%d:\n", testId + 1);
        _forn(p, 0, children.size()) {
            int x = children [p].first;
            int y = children [p].second;
            int u = res [p].first.first, d = res [p].first.second;
            int l = res [p].second.first, r = res [p].second.second;
            forn(i, x - u, x + d) {
                forn(j, y - l, y + r) {
                    game [i][j] = game [x][y];
                }
