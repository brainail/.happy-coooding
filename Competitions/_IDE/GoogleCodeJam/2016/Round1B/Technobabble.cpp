...
// MY
vector<pair<string, string>> tps;
int N;
int rtv;
bool used [1000 + 10];

void calc(int k = 0, int fk = 0) {
    if (k == N) {
        if (fk <= rtv) return;

        set<string> tfirst, tsecond;
        _forn(i, 0, N)
            if (used [i]) {
                tfirst.insert (tps [i].first);
                tsecond.insert (tps [i].second);
            }

        bool ok = true;
        _forn(i, 0, N)
            if (! used [i]) {
                if (! contain (tfirst, tps [i].first)) ok = false;
                if (ok && ! contain (tsecond, tps [i].second)) ok = false;
                if (! ok) break;
            }
        if (ok) rtv = fk;
