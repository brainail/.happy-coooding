...
bool aless(string a [3], string b [3]) {
    if (b [0] == "#") {
        return true;
    }
...
bool calc() {
    ...
    _forn(i, 0, 20)
        _forn(j, 0, 2) dp [i][j][0] = dp [i][j][1] = dp [i][j][2] = "#";
    dp [dl][0][0] = dp [dl][0][1] = dp [dl][0][2] = "";
    ...
    for (int i = dl; i >= 1; -- i)
        for (int j = 0; j < 2; ++ j) {
            ...
            _forn(da, 0, 10) {
                if (A [i - 1] != '?' && A [i - 1] != (char) (da + '0')) continue;

                _forn(db, 0, 10) {
                    if (B [i - 1] != '?' && B [i - 1] != (char) (db + '0')) continue;

                    int _da = da, _db = db;

                    _da -= j;
                    int newJ = 0;
                    if (_da < _db) ++ newJ, _da += 10;
                    int dr = _da - _db;

                    // 4? - 5?
                    if (! (i - 1) && newJ) continue;

                    string d_state [3];
                    d_state [0] = ((char) (dr + '0')) + dp [i][j][0];
                    d_state [1] = ((char) (da + '0')) + dp [i][j][1];
                    d_state [2] = ((char) (db + '0')) + dp [i][j][2];

                    if (aless(d_state, dp [i - 1][newJ])) {
                        dp [i - 1][newJ][0] = d_state [0];
                        dp [i - 1][newJ][1] = d_state [1];
                        dp [i - 1][newJ][2] = d_state [2];
                        continue;
                    }
                }
            }
...
int main() {
    ...
    forT(testId) {
        ...
        calc();
        swap(A, B);
        if (calc()) swap (rtv [1], rtv [2]);

        printf("Case #%d: %s %s\n", testId + 1, rtv [1].c_str(), rtv [2].c_str());
    }
