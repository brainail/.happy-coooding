...
const string INPUT_FILE
        = IDE_PATH + "GoogleCodeJam/2016/Round1A/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "GoogleCodeJam/2016/Round1A/output.txt";
...
vector<tvi> board;
int rows [100 + 10];
bool isHere [100 + 10][50 + 10][2500 + 10];
int Ja, Jb;
bool OK = false;
bool used [100 + 10];

inline bool checkRows (int k, int ja, int jb) {
    _forn(i, 0, k) {
        if (ja != -1 && Ja != -1 && ! isHere [rows [i]][ja][board [Ja][i]]) {
            return false;
        }
        if (jb != -1 && Jb != -1 && ! isHere [rows [i]][jb][board [Jb][i]]) {
            return false;
        }
    }
...
void walk(int lastI = -1, int kI = 0) {
    // early exit
    if (OK) return;
    ...
    if (kI == N) {
        _forn(jj, 0, N) {
            vector<int> column(N);
            _forn(ii, 0, N) column [ii] = board [rows [ii]][jj];
            ...
            if (! has) {
                _forn(jjj, 1, N) {
                    assert(column [jjj] > column [jjj - 1]);
                }
                _forn(jjj, 0, N) printf(" %d", column [jjj]);
                break;
            }
        }
            ...
            _forn(ja, 0, N) {
                ...
                if (Jb != -1) {
                    _forn(jb, ja + 1, N) {
                        ...
                        if (checkRows (kI + 1, ja, jb)) {
                            canGo = true;
                            break;
                        }
                    }
                } else {
                    if (checkRows (kI + 1, ja, -1)) {
                        canGo = true;
                        break;
                    }
                }
