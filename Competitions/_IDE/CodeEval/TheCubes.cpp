...
public:
    const int lev, x, y;
    Position(int _lev, int _x, int _y): lev(_lev), x(_x), y(_y) {}
};
...
void calcTest() {
    ...
    _forn(lev, 0, n)
        _forn(ii, 0, n)
              _forn(jj, 0, n) {
                  int flat_i = lev * n * n + ii * n + jj;
                  box [lev][ii][jj] = testCase [flat_i + (i + 1)];
                  bool isStart = '*' != box [lev][ii][jj] && !lev && (!ii || !jj || n-1 == ii || n-1 == jj);
                  fk [lev][ii][jj] = isStart ? 0 : -1;
                  if (isStart) {
                      q.push (new Position(lev, ii, jj));
                  }
              }
    ...
    while (!q.empty ()) {
        ...
        // low..
        if (cp->lev
            && 'o' == box [cp->lev][cp->x][cp->y]
            && '*' != box [cp->lev - 1][cp->x][cp->y]
            && fk [cp->lev - 1][cp->x][cp->y] < 0) {

                q.push (new Position(cp->lev - 1, cp->x, cp->y));
                fk [cp->lev - 1][cp->x][cp->y] = fk [cp->lev][cp->x][cp->y] + 1;
        }

        // high..
        if (cp->lev + 1 < n
            && 'o' == box [cp->lev + 1][cp->x][cp->y]
            && fk [cp->lev + 1][cp->x][cp->y] < 0) {
        ...
        // up..
        if (cp->x - 1 >= 0
            && '*' != box [cp->lev][cp->x - 1][cp->y]
            && fk [cp->lev][cp->x - 1][cp->y] < 0) {

            q.push (new Position(cp->lev, cp->x - 1, cp->y));
            fk [cp->lev][cp->x - 1][cp->y] = fk [cp->lev][cp->x][cp->y] + 1;
        }
        ...
        // left..
        if (cp->y - 1 >= 0
            && '*' != box [cp->lev][cp->x][cp->y - 1]
            && fk [cp->lev][cp->x][cp->y - 1] < 0) {

            q.push (new Position(cp->lev, cp->x, cp->y - 1));
            fk [cp->lev][cp->x][cp->y - 1] = fk [cp->lev][cp->x][cp->y] + 1;
        }
    ...
    _forn(ii, 0, n)
        _forn(jj, 0, n)
              if (fk [n - 1][ii][jj] >= 0)
                  if (!ii || !jj || n-1 == ii || n-1 == jj) {
                      if (rtv < 0 || fk [n - 1][ii][jj] < rtv) {
                          rtv = fk [n - 1][ii][jj];
                      }
                  }
