...
map<tll, int> previousCounts;
ordered_set previousNumbersSet;
list<tll> previousNumbers;

void calcTest() {
    tll n, k, a, b, c, r;
    sscanf(testCase.c_str(), "%lld,%lld,%lld,%lld,%lld,%lld", & n, & k, & a, & b, & c, & r);

    previousCounts.clear();
    previousNumbersSet.clear();
    previousNumbers.clear();

    tll m = a;
    forn(i, 1, k) {
        previousNumbers.pb(m);

        if (! contain(previousCounts, m)) {
            previousNumbersSet.insert(m);
            previousCounts [m] = 0;
        }
    ...
    for (int i = 0; i < goal; ++ i) {
        ...
        while (hi - lo > 1) {
            tll mi = (lo + hi) / 2;
            if (previousNumbersSet.order_of_key(mi + 1) < mi + 1) {
                hi = mi;
            } else {
                lo = mi;
            }
        ...
        tll nextNumber = hi;
        if (previousNumbersSet.order_of_key(lo + 1) < lo + 1) {
            nextNumber = lo;
        }

        tll removeNumber = previousNumbers.front();

        previousNumbers.pop_front();
        previousNumbers.pb(nextNumber);

        -- previousCounts [removeNumber];

        if (! previousCounts [removeNumber]) {
            previousCounts.erase(previousCounts.find(removeNumber));
            previousNumbersSet.erase(removeNumber);
        }

        if (! contain(previousCounts, nextNumber)) {
            previousNumbersSet.insert(nextNumber);
            previousCounts [nextNumber] = 0;
        }

        ++ previousCounts [nextNumber];
    }

    printf("%lld\n", previousNumbers.back());
}
...
int main(int argc, char * argv []) {
    ...
    while (getline(cin, testCase)) {
        testCase = deletews(testCase);
        if (testCase.empty()) break;
