...
void calcTest() {
    int n = clen(s);
    z.assign(n, 0);
    int max_subl = 0, from_p = -1;

    for (int from_i = 0; from_i < n; ++ from_i) {
        int l = 0, r = 0, subn = n - from_i;
        bool allSpaces = ' ' == s [from_i];
        int lastSpace = allSpaces ? 0 : -1;
        std::fill(z.begin(), z.begin() + subn, 0);

        for (int i = 1; i < subn; ++ i) {
            if (i <= r) z [i] = min (r - i + 1, z [i - l]);
            while (i + z [i] < subn && s [z [i] + from_i] == s [i + z [i] + from_i]) {
                ++ z [i];
            }
            ...
            int subl = min(z[i], i);
            if (! allSpaces && subl - 1 > lastSpace) {
                if (subl > max_subl) max_subl = subl, from_p = from_i;
            }

            if (' ' != s [i + from_i]) allSpaces = false;
            if (allSpaces) lastSpace = i;
        }
    }

    if (max_subl > 0) {
        for (int i = from_p; i < from_p + max_subl; ++ i) printf("%c", s [i]);
    } else {
        printf("NONE");
    }
