...
void calcTest() {
    int m = clen(board [0]), n = csize(board);

    dist.clear();
    dist.assign(n, vector<int> (m, -1));

    from.clear();
    from.assign(n, vector<tpii> (m, mp(-1, -1)));
    ...
    while (! q.empty()) {
        ...
        _forn(i, 0, 4) {
            int nx = state.first + v [i][0];
            int ny = state.second + v [i][1];
            if (nx >= 0 && ny >= 0 && nx < n && ny < m && isFree(nx, ny)) {
                board [nx][ny] = 'X';
                dist [nx][ny] = dist [state.first][state.second] + 1;
                from [nx][ny] = state;
                q.push(mp(nx, ny));
            }
        }
