...
string calcTest() {
    ...
    _forn(i, 0, clen(testCase)) {
        if ('0' == testCase [i] || '1' == testCase [i]) isHeader = false;
        if (isHeader) {
            header += testCase [i];
        } else if ('0' == testCase [i] || '1' == testCase [i]) {
            message += testCase [i];
        }
    }
    ...
    int codeLen = 1;
    map<string, char> c2s;
    while (headerIndex < clen(header)) {
        string scode = "";
        int xcode = code;
        while (xcode) {
            scode = toS(xcode % 2) + scode, xcode /= 2;
        }
        int e0 = codeLen - clen(scode);
        _forn(i, 0, e0) {
            scode = '0' + scode;
        }
        if (scode.find("0") == string::npos) {
            ++ codeLen, code = 0;
            continue;
        }
        c2s [scode] = header [headerIndex];
        ++ code, ++ headerIndex;
    }

    int messageIndex = 0;
    string rtv = "";
    while (messageIndex < clen(message)) {
        int ccodeLen = 0, base = pw2(2);
        _forn(i, 0, 3) {
            ccodeLen += base * (message [messageIndex] - '0');
            base /= 2, ++ messageIndex;
        }
        while (true) {
            string scode = message.substr ((unsigned long) messageIndex, (unsigned long) ccodeLen);
            messageIndex += ccodeLen;
            if (scode.find("0") != string::npos) {
                rtv += c2s[scode];
            } else {
                break;
