...
const static string tel [10] = {"0", "1", "abc", "def", "ghi", "jkl", "mno", "pqrs", "tuv", "wxyz"};

void combine(int ind, int gind) {
    if (ind == clen (number)) {
        if (comma) printf(",");
        comma = true;
        _forn (i, 0, ind) printf("%c", word [i]);
        return;
    }

    word [ind] = tel [number [ind] - '0'][gind];
    combine (ind + 1, 0);

    if (gind + 1 < clen (tel [number [ind] - '0'])) {
        combine (ind, gind + 1);
    }
}
...
void calcTest() {
    ...
    _forn(i, 0, clen (testCase))
        if (testCase [i] >= '0' && testCase [i] <= '9') {
            number += testCase [i];
        }

    word.resize (clen (number));
    comma = false;
