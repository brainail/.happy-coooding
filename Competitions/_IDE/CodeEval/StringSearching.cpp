...
bool calcTest() {
    ...
    string _sA, sA, _sB, sB;
    getline(sinput, _sA, ',');
    getline(sinput, _sB);
    int iA = 0, iB = 0, _lenA = clen (_sA), _lenB = clen (_sB);
    while (iA < _lenA) {
        if ('*' == _sA [iA]) _sA [iA] = '#';
        sA += _sA [iA ++];
    }
    while (iB < _lenB) {
        if ('\\' == _sB [iB] && iB + 1 < _lenB && '*' == _sB [iB + 1]) _sB [++ iB] = '#';
        sB += _sB [iB ++];
    }
    int lenA = clen (sA), lenB = clen (sB);
    if (!lenB || (!lenA && lenB == std::count(all(sB), '*'))) return true;
    vector<vector<bool>> can ((tul) (2), vector<bool> ((tul) lenA + 1, false));
    int lev = 1;
    _forn(i, 0, lenA) can [lev][i] = ('*' == sB [0] || sB [0] == sA [i]);
    _forn(k, 1, lenB) {
        int nev = lev^1;
        fill(all(can[nev]),false);
        _forn(i, 0, lenA - 1)
            if (can[lev][i] && '*' != sB[k]) can[nev][i + 1] = can[nev][i + 1] | (sB[k] == sA[i + 1]);
            else if (can[lev][i]) can[lev][i + 1] = can[nev][i + 1] = can[nev][i] = true;
        lev = nev;
    }
    bool rtv = false;
    _forn(i, 0, lenA) rtv |= can [lev][i];
    return rtv;
}
