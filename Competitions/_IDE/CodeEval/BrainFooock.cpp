...
inline int findAfterNext(int sIndex) {
    bool ok = false;
    int okValue = 0;
    ++ sIndex;
    ...
    while (sIndex < n) {
        ...
        switch (op) {
            ...
            case '<':
                ...
                break;
            case '+':
                memo [mIndex] = (memo [mIndex] + 1) % 256;
                break;
            case '-':
                memo [mIndex] = (256 + (memo [mIndex] - 1)) % 256;
                break;
            case '.':
                printf("%c", (char) memo [mIndex]);
                break;
            case '[':
                if (0 != memo [mIndex]) {
                    gates.pb(sIndex);
                } else {
                    sIndex = findAfterNext(sIndex);
                }
                break;
            case ']':
                sIndex = gates.back() - 1;
