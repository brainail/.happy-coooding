...
vector< string > board;
int n, m, maxs;
vector<pair<pair<int, int>, pair<int, int>>> maxr;
bool was [27];

inline void check(int s, int i, int lj, int ni, int rj) {
    if (s > maxs) {
        maxs = s;
        maxr.clear ();
    }
    if (s == maxs) {
        maxr.push_back (mp(mp(i, lj), mp(ni, rj)));
    }
}
...
void calcTest() {
    ...
    while (getline (cin, dataRow)) {
        ++ n;
        board.push_back (dataRow);
    }
    ...
    _forn(t, 0, csize(maxr)) {
        forn(i, maxr [t].first.first, maxr [t].second.first)
            forn(j, maxr [t].first.second, maxr [t].second.second)
                 board [i][j] = '*';
    }

    _forn(i, 0, n) {
        _forn(j, 0, m) printf("%c", board [i][j]);
        printf("\n");
    }
}
