...
const string INPUT_FILE
        = "/Users/brainail/DevProjects/Privacy/Programming/Happy-coding/Competitions/_IDE/input.txt";
const string OUTPUT_FILE
        = "/Users/brainail/DevProjects/Privacy/Programming/Happy-coding/Competitions/_IDE/output.txt";
...
inline double ccw_tr (Point & P0, Point & P1, Point & P2) {
    return ((P1.x - P0.x) * (P2.y - P0.y) - (P2.x - P0.x) * (P1.y - P0.y));
}

int wnForPoly (Point P, vector<Point> & V) {
    int wn = 0, n = csize (V) - 1;
    for (int i = 0; i < n; ++ i) {
        double ccw = ccw_tr (V[i], V[i + 1], P);
        
        if (0 == cmpf (ccw)) {
            double mi_x = min(V [i].x, V [i + 1].x), mi_y = min(V [i].y, V [i + 1].y);
            double ma_x = max(V [i].x, V [i + 1].x), ma_y = max(V [i].y, V [i + 1].y);
            if (P.x >= mi_x && P.x <= ma_x && P.y >= mi_y && P.y <= ma_y) return -1; // fake inside
        }
        
        if (V[i].y <= P.y) {
            if (V[i + 1].y > P.y && ccw > 0) ++ wn;
        } else {
            if (V[i + 1].y <= P.y && ccw < 0) -- wn;
        }
    }
...
bool calcTest() {
    ...
    string spoints, spoint;
    getline(sinput, spoints, '|');
    spoints [clen (spoints) - 1] = ',';
    istringstream spinput(spoints);
    string spx, spy;
    getline(sinput, spx, ' '); // useless space?
    if (0 == clen(spx)) {
        getline (sinput, spx, ' ');
    }
    getline(sinput, spy);
    double px = toF (spx), py = toF (spy);
    Point p = {px, py};
    vector<Point> points;
    while (true) {
        string sx, sy;
        while (0 == clen (sx) && getline (spinput, sx, ' '));
        if (0 == clen (sx)) break;
        getline (spinput, sy, ',');
        double x = toF (sx), y = toF (sy);
        Point vp = {x, y};
        points.pb (vp);
    }
    points.pb(points [0]);
    int wn = wnForPoly (p, points);
    return 0 == wn;
}
...
int main(int argc, char * argv []) {
    ...
    while (getline(cin, testCase)) {
        cout << (calcTest() ? "Citizen" : "Prisoner") << endl;
    }
    // CODE AREA <=
