...
inline void process_i(vector<char> & v, int & co, int mo, char ch) {
    if (mo) {
        _fordn (i, 9, co) v [i] = v [i - 1];
    }
    v [co] = ch;
    co = min (9, co + 1);
}
...
void process() {
    int i = 0, ro = 0, co = 0, mo = 0;
    while (i < clen (s)) {
        string cm = string (1, s [i ++]);
        if ('^' == s [i - 1]) cm += s [i ++];
        if (isDigit (s [i - 1]) && '^' == cm [0]) cm += s [i ++];

        if ("^c" == cm) {
            for(auto & v : t) v.assign (10, ' ');
        } else if ("^h" == cm) {
            ro = co = 0;
        } else if ("^b" == cm) {
            co = 0;
            ...
            std::fill(t [ro].begin () + co, t [ro].end (), ' ');
        } else if ("^i" == cm || "^o" == cm) {
            mo = "^o" == cm ? 0 : 1;
        } else if ("^^" == cm) {
            process_i(t [ro], co, mo, '^');
        } else if ('^' == cm [0]) {
            ro = cm [1] - '0', co = cm [2] - '0';
        } else {
            process_i(t [ro], co, mo, cm [0]);
        }
    }
}
