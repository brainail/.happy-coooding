...
// MY
typedef pair<pair<int, string>, int> vt;
...
public:
    bool operator() (vt x, vt y) {
        if (x.first.first == y.first.first) {
            int xln = clen (x.first.second);
            int yln = clen (y.first.second);
            if (xln == yln) return x.first.second > y.first.second;
            return xln < yln;
        }
...
void walk(int v, string code) {
    if (g [v][0].first.first < 0) {
        codes.pb(mp(g [v][0].first.second [0], code));
        return;
    }
    walk(g [v][0].second, code + '0');
    walk(g [v][1].second, code + '1');
}

void calcTest() {
    map<char, int> frequence;
    _forn(i, 0, clen(testCase)) {
        if (! frequence.count (testCase [i])) {
            frequence [testCase [i]] = 0;
        }
        ++ frequence [testCase [i]];
    }
    ...
    while (q.size () > 1) {
        ...
        vt rv = q.top ();
        q.pop ();
        int fr = lv.first.first + rv.first.first;
        string pivot = lv.first.second + rv.first.second;
        vt qp = mp(mp(fr, pivot), N ++);
        g [N - 1][0] = lv, g [N - 1][1] = rv;
        q.push(qp);
    }
    ...
    if (testIndex) printf("\n");
    ++ testIndex;
    _forn(i, 0, csize (codes)) {
        if (i > 0) printf(" ");
        printf("%c: %s;", codes [i].first, codes [i].second.c_str ());
    }
}
