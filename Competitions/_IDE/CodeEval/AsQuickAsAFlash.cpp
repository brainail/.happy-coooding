...
int aqaaf (int ll, int rr) {
    if (ll >= rr) return 0;

    int p_num = nums [ll], p_ind = ll;
    int ii = ll + 1, jj = rr;

    while (ii < jj) {
        while (ii < jj && nums [jj] >= p_num) -- jj;
        if (nums[jj] < p_num) {
            swap (nums [p_ind], nums [jj]);
            p_ind = jj;
        }

        if (ii >= jj) continue;

        while (ii < jj && nums [ii] <= p_num) ++ ii;
        if (nums [ii] > p_num) {
            swap (nums [p_ind], nums [ii]);
            p_ind = ii;
        }
    }

    return 1 + aqaaf (ll, p_ind - 1) + aqaaf (p_ind + 1, rr);
}
...
void calcTest() {
    ...
    string num;
    while (getline(sinput, num, ' ')) {
        nums.pb(toInt(num));
    }

    cout << aqaaf (0, csize(nums) - 1) << endl;
}
