...
const string INPUT_FILE
        = IDE_PATH + "Codechef/2017/SnackDown/PreElimB/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "Codechef/2017/SnackDown/PreElimB/output.txt";
...
vector< vector<int> > g;
vector<int> vdg;
vector< vector<int> > vdg2v;
set< pair<int, int> > eu;
vector< vector< pair<int, int> > > vdg2e;
vector<int> vpdsu;
vector<int> rtv;

int dsu_get (int v) {
    return (v == vpdsu [v]) ? v : (vpdsu [v] = dsu_get (vpdsu [v]));
}
...
int main() {
    ...
    forT(testId) {
        ...
        vdg2v.assign(nv, vector<int>());
        vdg2e.assign(nv, vector< pair<int, int> >());
        vpdsu.assign(nv, 0);
        eu.clear();
        _forn(i, 0, ne) {
            int from, to;
        ...
        _forn(idg, 0, nv) {
            _forn(j, 0, csize (vdg2v [idg])) {
                int from = vdg2v [idg][j];
                _forn(je, 0, csize (g [from])) {
                    int to = g [from][je];
                    pair<int, int> efrom = mp(from, to);
                    pair<int, int> eto = mp(to, from);
                    if (!eu.count (efrom)) {
                        vdg2e [idg].pb(efrom);
                        eu.insert (efrom);
                        eu.insert (eto);
        ...
        fordn(idg, nv - 1, 0) {
            rtv [idg] = rtve - 1;
            _forn(j, 0, csize (vdg2e [idg])) {
                pair<int, int> edge = vdg2e [idg][j];
                if (dsu_get (edge.first) != dsu_get (edge.second)) {
                    -- rtve;
                    dsu_join (edge.first, edge.second);
                }
            }
        }
        _forn(i, 0, nv) {
