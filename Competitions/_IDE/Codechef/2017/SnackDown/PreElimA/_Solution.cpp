...
const string INPUT_FILE
        = IDE_PATH + "Codechef/2017/SnackDown/PreElimA/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "Codechef/2017/SnackDown/PreElimA/output.txt";
...
int main() {
    ...
    forT(testId) {
        ...
        ml [1] = 0;
        forn(i, 2, n) {
            ml [i] = ml[i - 1] + (tll) (p[i] - (p[i - 1] + l)) * (tll) (i - 1);
        }
        mr [n] = 0;
        fordn(i, n - 1, 1) {
            mr [i] = mr[i + 1] + (tll) (p[i + 1] - (p[i] + l)) * (tll) (n - i);
        }

        tll ext_moves = -1;
        forn(i, 1, n) {
            tll mlr = ml[i] + mr[i];
            if (ext_moves < 0 || mlr < ext_moves) {
                ext_moves = mlr;
            }
        }
