...
const string INPUT_FILE
        = IDE_PATH + "Codechef/2017/SnackDown/Qualifier/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "Codechef/2017/SnackDown/Qualifier/output.txt";
...
int main() {
    ...
    forT(testId) {
        ...
        priority_queue< int, vector<int>, greater<int> > snakes;
        _forn(i, 0, N) {
            int L;
            scanf("%d", &L);
            snakes.push(L);
        ...
        _forn(i, 0, Q) {
            ...
            max_k [i] = snakes.top () >= queries [i].first
                       ? (int) snakes.size ()
                       : 0;
        }

        _forn(i, 0, Q) {
            queries [queries [i].second].first = max_k [i];
        }
