...
const string INPUT_FILE
        = IDE_PATH + "Codechef/2016/MayChallenge/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "Codechef/2016/AprilChallenge/output.txt";
...
void walk(int u, int p = -1) {
    ...
    _forn(i, 0, csize(g [u])) {
        tpii edge = g [u][i];
        if (edge.first == p) continue;
        mc = max (mc, f [edge.first] + edge.second);
    }
...
int main() {
    ...
    forT(testId) {
        ...
        _forn(i, 0, N - 1) {
            int vfrom, vto, cedge;
            scanf("%d %d %d", &vfrom, &vto, &cedge);
            -- vto, -- vfrom;
            g [vfrom].pb(mp(vto, cedge));
            g [vto].pb(mp(vfrom, cedge));
        }
    }
