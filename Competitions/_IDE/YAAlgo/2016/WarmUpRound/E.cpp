...
const string INPUT_FILE
        = IDE_PATH + "YAAlgo/2016/WarmUpRound/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "YAAlgo/2016/WarmUpRound/output.txt";
...
inline tll binpow (tll a, tll n) {
    tll res = 1;
    while (n) {
        if (n % 2) res = (res * a) % modulo;
        a = (a * a) % modulo, n /= 2;
    }
    return res;
}

inline bool ptest(tll x) {
    tll rtv = 0;
    _forn(i, 1, n + 1)
        if (a [i] % 2) {
            tll p = binpow(x, i);
...
int main() {
    ...
    forn(i, 1, (int) min((tll) 5000000 / n, (tll) (bm - 1)))
        if (ptest(i)) {
            printf("Yes");
            return 0;
        }
