...
const string INPUT_FILE
        = IDE_PATH + "RCC/2017/Round1A/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "RCC/2017/Round1A/output.txt";
...
void cs_fill(set<int> & c_set) {
    c_set.clear();
    forn(c, 1, k) c_set.insert (c);
}

void cv_fill(list<int> & c_list) {
    vector<int> c_vec(k);
    forn(c, 1, k) c_vec [c - 1] = c;
    forn(c, 1, k) {
        int x = rand() % k;
        int y = rand() % k;
        int buf = c_vec [x];
    ...
    c_list.clear ();
    _forn(i, 0, k) c_list.push_back (c_vec [i]);
}
    ...
    forT(testId) {
        ...
        _forn(i, 0, n) {
            _forn(j, 0, m) {
                ...
                bool ok = false;
                for (list<int>::iterator it = c_row.begin(); it != c_row.end(); ++ it) {
                    if (c_col [j].count (*it)) {
                        ok = true;
                        b [i][j] = *it;
                        c_row.erase (it);
