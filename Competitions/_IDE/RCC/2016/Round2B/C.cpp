...
const string INPUT_FILE
        = IDE_PATH + "RCC/2016/Round2B/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "RCC/2016/Round2B/output.txt";
...
// MY
long long hh [100 + 10][100 + 10];
vector<long long> p_pow;
const long long MODULO = 10000 + 7;
int mark [100 + 10][MODULO + 10];
vector<int> hmemo [100 + 10][MODULO + 10];
...
int main() {
    ...
    forT(testId) {
        ...
        p_pow.resize(n, 0);
        p_pow [0] = 1;
        _forn(i, 1, p_pow.size()) p_pow[i] = (p_pow[i-1] * p) % MODULO;

        fill0(hh);
        _forn(i, 0, n)
            _forn(j, i, n) {
                hh[i][j] = (s[j] - 'a' + 1) * p_pow[j - i];
                hh[i][j] %= MODULO;
        ...
        _forn(i, 3, n - 1) {
            _forn(j, i, n - 1) {
                tll h1 = hh [i][j];
                tll h2 = hh [j + 1][n - 1];
                if (cmark != mark [i][h1]) hmemo [i][h1].clear();
                hmemo [i][h1].pb(j + 1);
                mark [i][h1] = cmark;
                if (cmark != mark [i][h2]) hmemo [i][h2].clear();
                hmemo [i][h2].pb(j + 1);
                mark [i][h2] = cmark;
            }
        }
        ...
        _forn(i, 0, n - 2) {
            ...
            _forn(j, i + 1, n - 2) {
                ...
                _forn(q, j + 1, n - 2) {
                    ...
                    // h1
                    if (mark[t][h1] == cmark) {
                        vector< int > p = hh[t][h1];
                        _forn(pi, 0, csize (p)) pp.insert (p[pi]);
                    }

                    if (mark[t][h2] == cmark) {
                        vector< int > p = hh[t][h2];
                        _forn(pi, 0, csize (p)) pp.insert (p[pi]);
                    }

                    if (mark[t][h3] == cmark) {
                        vector< int > p = hh[t][h3];
                        _forn(pi, 0, csize (p)) pp.insert (p[pi]);
                    }
