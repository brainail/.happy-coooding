...
int main() {
  ...
  if (me > 0) {
    ...
    resMin = GetLL(me - 1);
    resMax = GetLL(me - 1);
    // int from_me = GetInt(me - 1);
    // fprintf(stderr, "> current node = %d, from node = %d\n", me, from_me);
  }

  resMin = min(resMin, vmin);
  resMax = max(resMax, vmax);
  // fprintf(stderr, "> current node = %d, resMin = %lld, resMax = %lld\n", me, resMin, resMax);
