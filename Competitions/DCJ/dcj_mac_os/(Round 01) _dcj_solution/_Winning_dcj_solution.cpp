...
// MY
tll calcSlow(vector<tll> players, int n = -1) {
  if (n < 0) n = csize(players);
  sort(all(players));
  _forn(i, 0, n) {
    bool nice = true;
    if (i - 1 >= 0 && players [i] == players [i - 1]) nice = false;
    if (i + 1 <= n - 1 && players [i] == players [i + 1]) nice = false;
    if (nice) return players [i];
  }
  return 0;
}
...
int main() {
  ...
  vector<tll> players(n);
  _forn(i, 0, n) players [i] = GetSubmission(i);
  tll winnerPlayer = calcSlow(players, n);
  printf("%lld\n", winnerPlayer);
