...
public class ShuntingYardExpressionsParser {
    ...
    static void processOperator(LinkedList<Integer> st, char op) {
        int r = st.removeLast(), l = st.removeLast();
        switch (op) {
        case '+':
            st.add(l + r);
            break;
    ...
    public static int eval(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (c == '(') { 
                ...
                while (!op.isEmpty() && priority(c) <= priority(op.getLast())) processOperator(st, op.removeLast());
                op.add(c);
            } else {
                String operand = "";
