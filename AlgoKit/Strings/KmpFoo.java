...
// Searching substring in O(N). 
// Knuth–Morris–Pratt algorithm + prefix function | Java
public class KmpFoo {
    ...
    // Another way is to join s with pattern by some unique char like '$' (e.g. s$pattern) 
    // and use only prefixFunction(...)
    public static int kmpMatcher(String s, String pattern) {
        ...
        for (int i = 0, k = 0; i < s.length(); i++) {
            ...
            if (pattern.charAt(k) == s.charAt(i)) ++k;
            if (k == m) return i + 1 - m;
        }
        return -1;
    }
