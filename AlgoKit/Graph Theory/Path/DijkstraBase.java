...
class DijkstraHelper {
    ...
    public int find(final int source, final int target) {
        ...
        while (true) {
            int bestVertex = NULO;
            for (int vertex = 0; vertex < vertices; ++ vertex) {
                if (!used [vertex] && (NULO == bestVertex || dist [vertex] < dist [bestVertex])) {
                    bestVertex = vertex;
                }
            }

            // Enough
            if (NULO == bestVertex) break;
            used [bestVertex] = true;

            for (final Edge edge : graph [bestVertex]) {
                if (dist [bestVertex] + edge.cost < dist [edge.to]) {
                    dist [edge.to] = dist [bestVertex] + edge.cost;
                    prev [edge.to] = bestVertex;
                }
            }
        }
        return dist [target];
...
public class DijkstraBase {
    ...
    public static void main(String [] args) {
        // new DijkstraBase().execute(ExecuteMode.STD);
        // new DijkstraBase().execute(ExecuteMode.FILE);
        new DijkstraBase().execute(ExecuteMode.IN_FILE);
    }
