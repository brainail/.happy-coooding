...
// MY
const int limit = 5e6;
const int nt = 1000000 + 10;

tvi vsqr(sqrt(limit) + 10);
bool good [limit + 10];
...
int main() {
    ...
    forn(i, 0, csize(vsqr)) vsqr [i] = i * i;
    // int co = 0;

    for (int m = 1; vsqr [m] <= limit; ++ m) {
        for (int n = 1 + (m & 1); (n < m) && (vsqr [n] + vsqr [m] <= limit); n += 2) {
            if (1 == gcd (n, m)) {
                for (int k = 1; k <= limit / (m * m + n * n); ++ k) {
                    int q = k * (vsqr [m] - vsqr [n]);
                    int t = k * (2 * m * n);
                    int N = k * (vsqr [m] + vsqr [n]);

                    int c = 2 * q;
                    if (c >= N && c <= 2 * N) good [N] = true;
                    c = 2 * t;
                    if (c >= N && c <= 2 * N) good [N] = true;
