...
public:
    ...
    SegTree(int aN) {
        // printf("--> Create seg tree\n");
        n = aN;
        t = new int [n * 2];
        m = new int [n * 2];
        q = new int [n * 2];
        h = new bool [n * 2];
        for (int i = n; i < 2 * n; i++) q[i] = 1;
        for (int i = 2 * n - 1; i > 1; i -= 2) q[i >> 1] = q[i] + q[i ^ 1];
    }

    void modifierHelper(int i, int p) {
        t[i] += p, m[i] += p, h[i] = true;
    }

    void pop(int i) {
        if (h[i >> 1]) {
            t[i >> 1] = max(t[i], t[i ^ 1]) + m[i >> 1];
        }
        else {
            t[i >> 1] = max(t[i], t[i ^ 1]);
        }
    ...
    void push(int i) {
        if (h[i >> 1]) {
            modifierHelper(i, m[i >> 1]), modifierHelper(i ^ 1, m[i >> 1]);
            h[i >> 1] = false, m[i >> 1] = 0;
        }
    }
    ...
    void modifyAdd(int a, int b, int v) {
        // printf("--> modifyAdd, a = %d, b = %d\n", a, b);
        ...
        for (; a <= b; a = (a + 1) >> 1, b = (b - 1) >> 1) {
            if ((a & 1) != 0) modifierHelper(a, v);
            if ((b & 1) == 0) modifierHelper(b, v);
        }
    ...
    int queryMax(int a, int b) {
        ...
        int res = -INF;
        for (; a <= b; a = (a + 1) >> 1, b = (b - 1) >> 1) {
            if ((a & 1) != 0) res = max(res, t[a]);
            if ((b & 1) == 0) res = max(res, t[b]);
        }
...
int query_up(int u, int v) {
    ...
    while (true) {
        ...
        if (uchain == vchain) {
            ...
            // query_tree(1, 0, ptr, posInBase[v] + (is_vhld ? 0 : 1), posInBase[u]+1);
            // Above is call to segment tree query function
            // if (qt[1] > ans) ans = qt[1]; // Update answer

            int qmax = tree->queryMax(posInBase[v] + (is_vhld ? 0 : 1), posInBase[u]);
            ans = max(ans, qmax);
        ...
        int qmax = tree->queryMax(posInBase[chainHead[uchain]], posInBase[u]);
        ans = max(ans, qmax);
...
int query(int u, int v) {
    ...
    */
    int lca = LCA(u, v);
    int first_path = query_up(u, lca); // One part of path
    int second_path = query_up(v, lca); // another part of path
    return max(first_path, second_path); // take the maximum of both paths
}
...
void subvchange(int u, int val) {
    assert(posInBase[u] + subsize[u] - 1 < ptr);
    tree->modifyAdd(posInBase[u], posInBase[u] + subsize[u] - 1, val);
}
...
void build() {
    ...
    tree = new SegTree(ptr);
    for (int i = 0; i < ptr; ++i) tree->set(i, baseArray[i]);
