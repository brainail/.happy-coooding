...
void BST< T, MAXN >::balance( Node *p )
{
    while( p )
    {
        ...
        p->depth = 1 + ( ldepth > rdepth ? ldepth : rdepth );
...
void BST< T, MAXN >::rotateL( Node *p )
{
    ...
    p->depth = 1 + ( ( p->l ? p->l->depth : 0 ) > ( p->r ? p->r->depth : 0 ) ? ( p->l ? p->l->depth : 0 ) : ( p->r ? p->r->depth : 0 ));
    q->cnt = 1 + ( q->l ? q->l->cnt : 0 ) + ( q->r ? q->r->cnt : 0 );
    q->depth = 1 + ( ( q->l ? q->l->depth : 0 ) > ( q->r ? q->r->depth : 0 ) ? ( q->l ? q->l->depth : 0 ) : ( q->r ? q->r->depth : 0 ) );
