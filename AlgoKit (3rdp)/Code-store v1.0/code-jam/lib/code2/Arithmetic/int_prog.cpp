 ...
 * Important Notes:
 *
 * 1. The first n columns of A must be lexicographically positive.  That
 *    is, the first non-zero element of the column must be positive.
 * 2. The above constraint says that the coefficients of the cost function
 *    must be non-negative (most likely positive anyway), so we can not
 *    just multiply the cost function by -1 to maximize!
 * 3. Suppose that the i-th coefficient of the cost function is negative,
 *    and there is a known upper bound U[i] for x[i] (there must be one if
 *    there is a minimum).  One can perform a change of variable
 *    x'[i] = U[i] - x[i].  This allows negative coefficients and hence
 *    the maximization problem can be solved.  Conversion must be done
 *    for the return values.
 * 4. If we want to have constraints that are >=, just multiply all the
 *    coefficients by -1.
 * 5. If we want to have constraints that are ==, do both >= and <=.
 * 6. The contents of A is destroyed after this routine.
 *
 */
