 ...
 * filling up residual[0..u-1][0..v-1].
 * result:
...
int coverU[MAXU],coverV[MAXV];
int wbipgraph[MAXU][MAXV];
...
wmatch(int u, int v) {
  ...
  /* make weighted bipartite the same on both sides */
  if (u < v) {
    for(i=u; i<v; i++) {
      for(j=0; j<v; j++) {
	wbipgraph[i][j] = 0;
  ...
  /* find feasible weighted cover */
  for(i=0; i<u; i++) {
    coverU[i] = 0;
    for(j=0; j<v; j++) {
      if (coverU[i] < wbipgraph[i][j]) {
	coverU[i] = wbipgraph[i][j];
      }
    }
  }
  ...
  /* adjust cover and matching until they meet */
  do {
    /* select edges that meet the cover */
    for(i=0; i<u; i++) {
      for(j=0; j<v; j++) {
	residual[i][j] = coverU[i]+coverV[j] - wbipgraph[i][j];
    ...
    /* see if we can adjust the cover to include edge between vertices
       not in the max matching */
    eps = -1;
    for(i=0; i<u; i++) {
      /* consider u's that are not in the matching */
      if (flagUvisited[i]) {
	for(j=0; j<v; j++) {
	  /* consider v's that are not in the matching */
	  if (!flagVvisited[j] && 
	      (eps == -1 || eps > residual[i][j])) {
	    eps = residual[i][j];
	  }
	}
      }
...
int
main() {
  int i,j,u,v,setnum,sum;

  setnum=0;
  while( scanf("%d %d", &u, &v) == 2 ) {
    for( i = 0; i < u; i++ ) {
      for( j = 0; j < v; j++ ) {
	scanf("%d",&wbipgraph[i][j]);
      }
    }
