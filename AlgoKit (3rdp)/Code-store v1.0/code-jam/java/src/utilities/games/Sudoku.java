...
public class Sudoku
{
    ...
    static void marcar(int i, int j, int n)
    {
        ...
        for (int a = 0; a < tS2; a++) {
            if (a != j) sudoku[i][a] &= mascara;
            if (a != i) sudoku[a][j] &= mascara;
        }
        for (int a = iMenor; a < iMayor; a++)
            for (int b = jMenor; b < jMayor; b++)
                if (a != i || b != j) sudoku[a][b] &= mascara;
    }
        ...
        int[] visitados)
    {
        while (true) {
            int iActual = 0;
            forExterno: for (int i = 0; i <= tS2; i++) {
                if (tamGrupos[i] == -1) break;
            ...
            forExterno: for (int i = 0; i <= tS2; i++) {
                ...
                for (int mascara = 1; mascara < mascaraActual; mascara++) {
                    ...
                    for (int j = 0; j < tamActual; j++) {
                        if ((temp & 1) == 1)
                            m |= sudoku[zona[grupos[iActual + j]][0]][zona[grupos[iActual
                                            + j]][1]];
                        temp >>= 1;
                    }
                    int tamCuenta = Integer.bitCount(m);
                    if (tamCuenta == tamActual) continue;
                    if (visitados[m] == idMascara) continue;
                    temp = ~m;
                    int cuenta = 0;
                    for (int j = 0; j < tamActual; j++) {
                        int valor = sudoku[zona[grupos[iActual + j]][0]][zona[grupos[iActual
                                        + j]][1]];
                        if ((valor & temp) == 0) cuenta++;
                    }
                    if (cuenta == tamCuenta) {
                        int posA = 0;
                        for (int j = 0; j < tamActual; j++) {
                        ...
                        for (int j = 0; j < tamActual; j++) {
                            ...
                            if ((valor & temp) != 0) {
                                actuales[posA++] = grupos[iActual + j];
                                sudoku[zona[grupos[iActual + j]][0]][zona[grupos[iActual
                                                + j]][1]] &= temp;
                            }
                        }
        ...
        while (sumMejor > 1 && sumAnterior != sumMejor) {
            ...
            for (int i = 0; i < tS2; i++)
                for (int j = 0; j < tS2; j++) {
                    ...
                    if (sudoku[i][j] <= todos) termino = false;
            ...
            if (sumMejor != 1) limpiar(sudoku, grupos, tamGrupos);
        }
        if (termino) {
            for (int i = 0; i < tS2; i++) {
                for (int j = 0; j < tS2; j++) {
