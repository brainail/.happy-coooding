    ...
    public String decodeAtIndex(String s, int k) {
        ...
        for (int i = 0; i < n; ++i) {
            if (Character.isDigit(text[i])) {
                ...
                len += word.length();
                len *= r;
                ops.push(new Op(word.toString(), r));
        ...
        while (!ops.isEmpty()) {
            ...
            k %= len;
            k = 0 == k ? (int) len : k;
            long beforeAdd = len - op.add.length();
            if (k > beforeAdd) {
                return "" + op.add.charAt((int) (k - beforeAdd - 1));
            }
            len = beforeAdd;
        }
        return "";
